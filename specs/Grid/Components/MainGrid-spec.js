...
describe("Grid.Components.MainGrid", function() {
    ...
    it("should render a grid", function() {
        ...
        var navDomNode = domNode.children[0];
        expect(navDomNode.tagName).toEqual('NAV');
        expect(navDomNode.classList.contains('grid-toolbar')).toBe(true);
        var gridDomNode = component.refs.gridNode.getDOMNode();
        expect(gridDomNode.classList.contains('grid')).toBe(true, 1);
        expect(gridDomNode.classList.contains('grid-main')).toBe(true, 2);
    ...
    it("should have placeholders when going in dragging mode", function(done) {
        var element = React.createElement(MainGrid, {node: testGrid});
        var component = componentUtils.renderIntoDocument(element);
        expect(component.getDOMNode().classList.contains('grid-component-design-mode')).toBe(false);
    ...
    it("should navigate through module cells", function() {
        ...
        var mainGridProto = jasmineReact.classPrototype(MainGrid);
