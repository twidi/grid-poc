...
var Manipulator = {
     ...
     */
    getModuleCellFromRow: function(row, atBottom, atRight) {
     ...
     * @return {XML} - The asked cell
     */
    getModuleCellFromCell: function(cell, atBottom, atRight) {
     ...
     * @return {XML} - The asked cell
     */
    getNextHorizontalCell: function(cell, toRight, forceAtBottom) {
        if (!cell) { return; }
        var row = cell.parentNode;
        // we will force `forceAtBottom` if the row we are in is the last one in a multi-row cell
        var isAtBottom = forceAtBottom || (row && row.parentNode && row.parentNode.children.length > 1 && row == row.parentNode.lastChild);
        var sibling = toRight ? cell.nextSibling : cell.previousSibling;
        if (sibling) {
     ...
     * @return {XML} - The asked cell
     */
    getNextVerticalCell: function(cell, toBottom, forceAtRight) {
        if (!cell) { return; }
        var row = cell.parentNode;
        if (!row) { return; }
        // we will force `forceAtRight` if the cell we are in is the last one in a multi-cells row
        var isAtRight = forceAtRight || (row.children.length > 1 && cell == row.lastChild);
        var sibling = toBottom ? row.nextSibling : row.previousSibling;
        if (sibling) {
            return this.getModuleCellFromRow(sibling, !toBottom, isAtRight);
        }
        var parentGrid = this.getNearestGrid(row);
        if (parentGrid) {
